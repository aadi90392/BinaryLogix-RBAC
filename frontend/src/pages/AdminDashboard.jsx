...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      <nav className="bg-slate-900 shadow-md px-8 py-4 flex justify-between items-center text-white">
        <h1 className="text-xl font-bold tracking-tight flex items-center gap-2"> Admin Portal</h1>
        <div className="flex gap-4">
          <button onClick={() => setActiveTab('directory')} className={`px-4 py-2 rounded-lg transition font-medium ${activeTab === 'directory' ? 'bg-blue-600' : 'hover:bg-slate-800'}`}>Directory</button>
          <button onClick={() => setActiveTab('profile')} className={`px-4 py-2 rounded-lg transition font-medium ${activeTab === 'profile' ? 'bg-blue-600' : 'hover:bg-slate-800'}`}>My Profile</button>
          <button onClick={() => {localStorage.clear(); navigate('/');}} className="bg-red-600 px-4 py-2 rounded-lg font-medium hover:bg-red-700">Logout</button>
        </div>
      </nav>
        ...
        {activeTab === 'directory' ? (
          <>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-slate-900">System Records</h2>
              <button onClick={() => setShowAddForm(!showAddForm)} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-bold shadow-sm hover:bg-blue-700 transition">
                {showAddForm ? 'Close Form' : '+ Add Manager/User'}
              </button>
            </div>
            {showAddForm && (
              <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
                <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <select value={formData.role} onChange={(e)=>setFormData({...formData, role:e.target.value})} className="p-2.5 border rounded-lg bg-slate-50 font-bold focus:ring-2 focus:ring-blue-500">
                    <option value="User">User</option>
                    <option value="Manager">Manager</option>
                  </select>
                  <button type="submit" className="bg-green-600 text-white font-bold rounded-lg hover:bg-green-700">Save Account</button>
                </form>
              </div>
            )}
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-6 mb-8">
              ...
              <div><h2 className="text-3xl font-bold">{myProfile?.name}</h2><span className="inline-block mt-1 px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold uppercase tracking-widest">{myProfile?.role}</span></div>
            </div>
